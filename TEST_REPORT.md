...
- Nine app-isolation policy cases: distinct Spotify/Discord, Spotify/Zoom, and Spotify/Roblox selections; rejection of same-browser, parent/helper in both directions, OpenYap self-capture, and empty source.
...
## 1.2.2 — expanded local collection and library refresh

Added 21 more sourced recordings locally (42 total), including recorded door knocks and Discord call/join/leave sounds. All 42 decoded successfully, were non-silent, fit the 15-second limit, and persisted after reopening the library; no audio was played through the live session. Library tests passed for an already-open library detecting files added by another instance, default-hidden demos, import persistence, and safe removal. Release build and strict signature verification passed. Expanding the soundboard now calls library refresh. No DSP/routing changes were made. Live UI/audio checks remain deferred to avoid interrupting the active call.

## 1.3.0 — shared music/effects gain and curation

Music and effects now pass through the same send gain before microphone is mixed in. Both UI slider locations bind to the same value and mute; both lock under Max sharing. The existing Max boost now applies to both sources. Removed the independent soundboard send setting. Soundboard and existing DSP/routing suites passed under sanitizers, including shared-level scaling, Max gain on effects, immediate shared mute, unchanged microphone contribution, independent local listening, stop-only-effects behavior, clipping protection, and feedback exclusion. Import/library tests and release compilation passed. Strict signature verification passed. No live audio or UI manipulation was used. The nine rejected recordings and five similar newer spoken clips were moved into a local backup outside the active library (28 remain); none were permanently deleted. The running OpenYap process is preserved during installation. Live slider/pad verification remains deferred.

## 1.5.0 — typed speech, 2026-09-20

Added local AVSpeechSynthesizer buffer rendering and Natural/Deep/Tiny styles under Soundboard. Speech feeds the existing effect slots, so local gain/master and shared send gain/mute/Max/output protection are unchanged. Request generations reject stale completions; Stop, a new pad, session shutdown and interruption cancel pending synthesis. No direct speaker playback or default-device changes are used for synthesis.

Validation: release compilation; rendered Samantha speech at all three pitches into nonzero, finite, bounded stereo 48 kHz PCM; empty/over-limit text, unavailable voice, overlong audio, cancellation, and rendering again after cancellation. Existing offline soundboard DSP tests verify shared gain/mute/Max, separate listening gain, output protection, resampling and Stop preserving music. Speech service tests require an unrestricted local process; restricted-sandbox rendering timed out as expected and produced a recoverable error.

Hands-on verification still needed: subjective voice/style quality, UI layout with every installed voice, and another participant hearing speech in Roblox or another receiving app. No live game/call was interrupted for testing. The preexisting session cache capacity also applies to speech.
